...
{{
  ...
  "reason": "<để trống nếu confidence >= 0.8>"
    ...
    return {**state, "confidence": confidence, "is_fallback": is_fallback,
            "fallback_reason": reason, "suggestions": suggestions}
...
2. Mapping: liệt kê từng thành phần kỹ thuật của "{state['concept']}" ứng với gì trong "{interest}".
   Đảm bảo không bỏ sót thành phần quan trọng nào.
